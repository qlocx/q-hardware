...
void qlocx_log_clear_until_hash(uint8_t* hash)
{
  ...
  do {
    ...
    // check if the has matches
    if (qlocx_flash_cmp(page_index, byte_index + 6, hash, HASH_SIZE) == 0) {
      found_hash = true;
      NRF_LOG_INFO("Hash found at index:%d", i);
    }
    // if the entry is not empty, we check if it is above found hash and save it
    if (!qlocx_flash_is_empty(page_index, byte_index, 6)) {
      // save entry
      if (found_hash) {
        qlocx_flash_get(page_index, byte_index, buffer + saved_entries
          * QLOCX_LOG_ENTRY_SIZE, QLOCX_LOG_ENTRY_SIZE);
        saved_entries++;
        NRF_LOG_INFO("Log entry at index:%d saved", i);
      }
    }
    i++;
  } while (i < QLOCX_LOG_MAX_ENTRIES);
