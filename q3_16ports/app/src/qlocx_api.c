...
bool qlocx_api_open_port(qlocx_ble_t* context)
{
  ...
  // NRF_LOG_INFO("pcb date: År %d Månad %d Dag %d Tid %d:%d", current_date.year, current_date.month, current_date.day, current_date.hour, current_date.minute);
...
bool qlocx_api_purge_logs_until_hash(qlocx_ble_t* context)
{
  ...
  memcpy(hash, context->rx_buffer+1, QLOCX_LOG_ENTRY_SIZE);
